...
export function initComposer(api) {
  ...
  // Start screen: type a few words, watch them land on a cookie, carry it into the designer.
  (function sayIt() {
    const input = $("sayItInput"),
      stage = $("heroCookie"),
      looks = $("sayItLooks");
    if (!input || !stage || !looks) return;
    const lookIds = ["confetti", "fireworks", "rose", "hearts", "sunshine", "tech-prism"];
    let look = lookIds[0];
    const spec = () => {
      const b = backgrounds.find((x) => x.id === look);
      return {
        backdrop: { id: b.id, color: b.color, accent: b.accent || null, image: null },
        text: { ...defaultText(), message: input.value.trim(), font: "classic", color: b.ink, size: 0.105 },
        shape: "square",
        view: { zoom: 1, x: 0, y: 0, fit: "cover" },
      };
    };
    const footerButton = $("continueButton");
    let footerLabel = null;
    const paint = () => {
      const live = input.value.trim().length > 0;
      document.body.classList.toggle("say-it-live", live);
      stage.hidden = !live;
      if (live) drawCookie(stage, spec());
      // The sticky bar button follows the cookie the visitor is making.
      if (live && footerLabel === null) {
        footerLabel = footerButton.innerHTML;
        footerButton.innerHTML = 'Make this cookie <span aria-hidden="true">→</span>';
      } else if (!live && footerLabel !== null) {
        footerButton.innerHTML = footerLabel;
        footerLabel = null;
      }
    };
    for (const id of lookIds) {
      ...
      dot.type = "button";
      dot.className = "say-it-look";
      dot.dataset.look = id;
      dot.title = b.name;
      dot.setAttribute("aria-label", b.name);
      dot.setAttribute("aria-pressed", String(id === look));
      const chip = canvas(96);
      chip.setAttribute("aria-hidden", "true");
      drawBackground(chip.getContext("2d"), 96, { id: b.id, color: b.color, accent: b.accent || null, image: null });
      dot.append(chip);
      dot.addEventListener("click", () => {
        look = id;
        looks.querySelectorAll("button").forEach((x) => x.setAttribute("aria-pressed", String(x === dot)));
        paint();
      });
      looks.append(dot);
    }
    const go = () => {
      if (!input.value.trim()) {
        input.focus();
        input.classList.remove("say-it-nudge");
        void input.offsetWidth;
        input.classList.add("say-it-nudge");
        return;
      }
      const made = spec();
      document.body.classList.remove("say-it-live");
      stage.hidden = true;
      footerLabel = null;
      api.start(made);
      chooseTab("words");
    ...
    $("sayItGo").addEventListener("click", go);
    footerButton.addEventListener(
      "click",
      (e) => {
        if (!document.body.classList.contains("say-it-live") || document.body.dataset.step !== "upload") return;
    ...
    // Rotating examples in the empty box show what people write.
    const examples = ["Happy 40th, Dave", "Thank you, team", "Suzy turns ONE", "We shipped it", "Will you marry me?", "Best mom ever"];
    let n = 0;
    setInterval(() => {
      if (document.activeElement === input || input.value) return;
      n = (n + 1) % examples.length;
      input.placeholder = examples[n];
    }, 2600);
  })();
